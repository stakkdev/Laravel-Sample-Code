...
<div class="form-group row align-items-center" :class="{'has-danger': errors.has('permissions'), 'has-success': fields.permissions && fields.permissions.valid }">
    <label for="permissions" class="col-form-label text-md-right" :class="isFormLocalized ? 'col-md-4' : 'col-md-2'">{{ trans('admin.role.columns.permissions') }}</label>
    <div :class="isFormLocalized ? 'col-md-4' : 'col-md-9 col-xl-8'">
        <multiselect v-model="form.permissions" placeholder="{{ trans('brackets/admin-ui::admin.forms.select_options') }}" label="name" track-by="id" :options="{{ $permissions->toJson() }}" :multiple="true" open-direction="bottom"></multiselect>
